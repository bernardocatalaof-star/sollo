...
Rules (per cabin, per stay):
  ...
  - a booking with Booking.skip_landowner_fees owes neither fee -- either a manual
      no-show override, or the Sheet reporting the booking as cancelled -- since the
      cabin was never actually used. Revenue still counts in both cases (that's
      tracked on the booking itself, not here).
...
def calculate_booking_fees(booking: Booking) -> BookingFees:
    ...
    if booking.skip_landowner_fees:
        # No-show or cancelled: the cabin was never actually used, so neither
        # the cleaning fee nor the land (overnight) fee is owed to the
        # landowner -- only the booking's own revenue still counts (handled
        # separately, not here).
